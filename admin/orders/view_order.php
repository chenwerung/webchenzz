...
<div class="card card-outline card-dark shadow rounded0-0">
    <div class="card-header">
        <h3 class="card-title"><b>Detail Pesanan</b></h3>
        <div class="card-tools">
            <button class="btn btn-primary btn-flat btn-sm" type="button" id="update_status"><i class="fa fa-edit"></i> Update Status</button>
            <button class="btn btn-danger btn-flat btn-sm" type="button" id="delete_order"><i class="fa fa-trash"></i> Hapus</button>
            <a class="btn btn-default btn-flat border btn-sm" href="./?page=orders"><i class="fa fa-angle-left"></i> Kembali</a>
        </div>
    </div>
    <div class="card-body">
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-md-6">
                    <label for="" class="text-muted">Tanggal Pemesanan</label>
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="ml-3">
                        <?php if(isset($status)): ?>
                            <?php if($status == 0): ?>
                                <span class="badge badge-secondary px-3 rounded-pill">Tertunda</span>
                            <?php elseif($status == 1): ?>
                                <span class="badge badge-primary px-3 rounded-pill">Dikemas</span>
                            <?php elseif($status == 2): ?>
                                <span class="badge badge-success px-3 rounded-pill">Untuk dikirim</span>
                            <?php elseif($status == 3): ?>
                                <span class="badge badge-warning px-3 rounded-pill">Dalam perjalanan</span>
                            <?php elseif($status == 4): ?>
                                <span class="badge badge-default bg-gradient-teal px-3 rounded-pill">Terkirim</span>
                            <?php else: ?>
                                <span class="badge badge-danger px-3 rounded-pill">Dibatalkan</span>
                            <?php endif; ?>
                        <?php else: ?>
                            N/A
                        <?php endif; ?>
...
<script>
    $(function(){
        $('#update_status').click(function(){
            uni_modal("Update Status Pesanan","orders/update_status.php?id=<?= isset($id) ? $id :'' ?>")
        })
        $('#btn-cancel').click(function(){
            _conf("Yakin akan membatalkan Pesanan?","cancel_order",[])
        })
        $('#delete_order').click(function(){
            _conf("Yakin akan menghapus pesanan ini permanen?","delete_order",[])
        })
    })
